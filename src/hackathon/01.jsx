...
const HackathonsPage = () => {
  return (
    <div className="min-h-screen bg-transparent text-slate-50 relative selection:bg-slate-700 selection:text-white antialiased font-sans">
      {/* Background Effects */}
      ...
      <main className="container mx-auto px-4 sm:px-6 py-8 sm:py-12 relative z-10 max-w-6xl">
        {/* Header */}
        <Link to="/" className="group inline-flex items-center gap-2 text-xs sm:text-sm font-medium text-slate-400 transition-colors hover:text-white mb-6 sm:mb-8">
          <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
          Back to home
        </Link>

        <header className="border-b border-slate-800/80 pb-8 sm:pb-10 text-center">
          <SectionLabel>Championship Record</SectionLabel>
          <h1 className="mt-2 text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-white leading-tight">
            Hackathons &amp; Winnings
          </h1>
          <p className="mt-3 text-slate-400 text-xs sm:text-base max-w-2xl mx-auto">
            High-pressure engineering sprints, hardware builds, and competitive autonomous robotics challenges.
          </p>
        </header>
        ...
        <div className="flex flex-col gap-6 sm:gap-8">
          {hackathons.map((hackathon) => (
            <motion.div
              ...
              className="group relative bg-[#0a0e1a]/70 border border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden hover:border-slate-600 hover:shadow-2xl transition-all duration-300 flex flex-col md:flex-row gap-6 sm:gap-8 md:gap-12 p-5 sm:p-8 md:p-10"
            >
              {/* Hover Glow */}
              ...
              <div className="md:w-5/12 shrink-0 flex flex-col justify-between relative z-10">
                <div className="space-y-3 sm:space-y-4">
                  <div className="flex items-center gap-2.5">
                    <span className="text-2xl sm:text-3xl select-none">{hackathon.medal}</span>
                    <span className="text-[10px] sm:text-xs font-mono font-bold text-amber-300 bg-amber-500/10 px-2.5 py-1 rounded border border-amber-500/20 uppercase tracking-wider select-none">
                      {hackathon.position} • {hackathon.prize}
                    </span>
                  </div>

                  <div>
                    <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight text-slate-100 group-hover:text-cyan-400 transition-colors leading-tight">
                      {hackathon.name}
                    </h2>
                    <p className="text-xs text-slate-500 font-mono tracking-wider uppercase mt-1.5 sm:mt-2 select-none">
                      {hackathon.hackathon}
                    </p>
                  </div>
                ...
                <div className="mt-5 sm:mt-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2">
                    {hackathon.tech.map((t, i) => (
                      <span key={i} className="text-[11px] sm:text-xs font-mono font-medium px-2.5 py-1 rounded bg-slate-800 border border-slate-700 text-slate-300 select-none">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="md:w-7/12 flex flex-col justify-between border-t md:border-t-0 md:border-l border-slate-800/80 pt-5 md:pt-0 md:pl-8 lg:pl-10 relative z-10">
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-500 font-mono mb-1.5 sm:mb-2 select-none">
                      Overview
                    </h4>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {hackathon.challenge && (
                      <div className="py-2.5 sm:py-3 px-3.5 sm:px-4 bg-slate-950/60 border border-slate-800 rounded-xl">
                        <span className="text-[9px] font-mono text-slate-400 uppercase block mb-1 tracking-wider select-none font-bold">Engineering Challenge</span>
                        <span className="text-xs sm:text-sm text-slate-400 leading-relaxed block">{hackathon.challenge}</span>
                      </div>
                    )}
                    {hackathon.outcome && (
                      <div className="py-2.5 sm:py-3 px-3.5 sm:px-4 bg-slate-900/60 border border-slate-800 rounded-xl">
                        <span className="text-[9px] font-mono text-cyan-400 uppercase block mb-1 tracking-wider select-none font-bold">Key Outcome</span>
                        <span className="text-xs sm:text-sm text-slate-300 leading-relaxed block">{hackathon.outcome}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {hackathon.links && (
                  <div className="flex flex-wrap justify-start sm:justify-end gap-2.5 sm:gap-3 pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-slate-800/60">
                    {hackathon.links.code && (
                      <a href={hackathon.links.code} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 sm:px-4.5 py-2 rounded-xl text-xs font-semibold bg-slate-900 text-slate-300 hover:text-white hover:bg-slate-800 transition-colors border border-slate-800 cursor-pointer">
                        <Github className="w-4 h-4" /> Hardware Source
                      </a>
                    )}
                    {hackathon.links.demo && (
                      <a href={hackathon.links.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3.5 sm:px-4.5 py-2 rounded-xl text-xs font-semibold bg-white text-slate-950 hover:bg-slate-200 transition-colors shadow-md cursor-pointer">
