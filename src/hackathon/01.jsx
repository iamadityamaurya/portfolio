    ...
    <div className="min-h-screen bg-transparent text-slate-50 relative selection:bg-slate-700 selection:text-white antialiased font-sans">
      ...
      <main className="container mx-auto px-6 py-12 relative z-10 max-w-6xl">
        ...
        <header className="mt-12 border-b border-slate-800/80 pb-10 text-center">
          <h1 className="mt-4 text-4xl font-extrabold tracking-tight text-white leading-tight md:text-6xl">
            Hackathons &amp; Winnings
          </h1>
        </header>
        ...
        <div className="flex flex-col gap-8">
          {hackathons.map((hackathon) => (
            <motion.div
              ...
              className="group relative bg-[#0a0e1a]/70 border border-slate-800 rounded-3xl overflow-hidden hover:border-slate-600 hover:shadow-2xl transition-all duration-300 flex flex-col md:flex-row gap-8 md:gap-12 p-8 md:p-10"
            >
              {/* Hover Glow */}
              ...
              <div className="md:w-5/12 shrink-0 flex flex-col justify-between relative z-10">
                <div className="space-y-4">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs font-mono font-bold text-amber-300 bg-amber-500/10 px-2.5 py-1 rounded border border-amber-500/20 uppercase tracking-wider select-none">
                      {hackathon.position} • {hackathon.prize}
                    </span>
                  </div>

                  <div>
                    <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight text-slate-100 group-hover:text-cyan-400 transition-colors leading-tight">
                      {hackathon.name}
                    </h2>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {hackathon.tech.map((t, i) => (
                      <span key={i} className="text-xs font-mono font-medium px-2.5 py-1 rounded bg-slate-800 border border-slate-700 text-slate-300 select-none">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="md:w-7/12 flex flex-col justify-between border-t md:border-t-0 md:border-l border-slate-800/80 pt-6 md:pt-0 md:pl-10 relative z-10">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {hackathon.challenge && (
                      <div className="py-3 px-4 bg-slate-950/60 border border-slate-800 rounded-xl">
                    ...
                    {hackathon.outcome && (
                      <div className="py-3 px-4 bg-slate-900/60 border border-slate-800 rounded-xl">
                        ...
                        <span className="text-xs md:text-sm text-slate-300 leading-relaxed block">{hackathon.outcome}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {hackathon.links && (
                  <div className="flex justify-end gap-3 pt-6 mt-6 border-t border-slate-800/60">
                    ...
                    {hackathon.links.demo && (
                      <a href={hackathon.links.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-4.5 py-2 rounded-xl text-xs font-semibold bg-white text-slate-950 hover:bg-slate-200 transition-colors shadow-md cursor-pointer">
